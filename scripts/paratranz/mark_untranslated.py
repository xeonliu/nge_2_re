...
def process_report(report_path: str, auth: str, dry_run: bool = False) -> dict:
    ...
    """
    stats = {"total": 0, "marked": 0, "failed": 0, "not_found": 0}
    ...
    for idx, entry in enumerate(entries, 1):
        ...
        if dry_run:
            print(
                f"[{idx}/{len(entries)}] [DRY RUN] Would mark {string_id}: {original[:50]}..."
            )
            stats["marked"] += 1
        else:
            if mark_as_untranslated(string_id, auth):
                print(
                    f"[{idx}/{len(entries)}] ✓ Marked {string_id}: {original[:50]}..."
                )
                stats["marked"] += 1
                if error:
                ...
                print(
                    f"[{idx}/{len(entries)}] ✗ Failed to mark {string_id}: {original[:50]}..."
                )
                stats["failed"] += 1
...
def main():
    ...
    total_stats = {"total": 0, "marked": 0, "failed": 0, "not_found": 0}
